...
func intcode_loop(src []int, inputs []int, start int) (int, int, int) {
    inpInd, progInd := 0, start
    for ; progInd < len(src); {
        var op, val int
...
func chain_loops(src []int, inputs []int) int {
    progCopy := make([][]int, 5)
    progIndex := make([]int, 5)
    for i := 0; i < 5; i++ {
        progCopy[i] = make([]int, len(src))
        copy(progCopy[i], src)
        progIndex[i] = 0
    }
    ...
    for i := 0; i < 5; i++ {
        var index int
        nextInp, _, index = intcode_loop(progCopy[i], []int{inputs[i], nextInp}, progIndex[i])
        progIndex[i] = index
    }
    output = nextInp
    
    for i := 0; stopCode != 99; i = (i + 1) % 5 {
        var index int
        nextInp, stopCode, index = intcode_loop(progCopy[i], []int{nextInp}, progIndex[i])
        progIndex[i] = index
        if i == 4 {
            output = nextInp
        }
