...
func increaseDist(wires []Wire, pts []Point, dists []int) {
    totalDist := 0
    for _, w := range wires {        
        for inter := 0; inter < len(pts); inter++{
            if w.vert() && w.X1 == pts[inter].X && diffProd(pts[inter].Y, w.Y1, w.Y2) <= 0 {
                dists[inter] += totalDist + abs(pts[inter].Y - w.Y1)
            } else if !w.vert() && w.Y1 == pts[inter].Y && diffProd(pts[inter].X, w.X1, w.X2) <= 0 {
                dists[inter] += totalDist + abs(pts[inter].X - w.X1)
            }
        }
        totalDist += abs(w.X2 - w.X1) + abs(w.Y2 - w.Y1)
    }
}
    ...
    for ind, w1 := range wire1 {
        for _, w2 := range wire2[ind:] {
            ...
            if crossX != 0 || crossY != 0 {
                inters = append(inters, Point{crossX, crossY})
            }
        }
    }
    
    lengths := make([]int, len(inters))
    increaseDist(wire1, inters, lengths)
    increaseDist(wire2, inters, lengths)
