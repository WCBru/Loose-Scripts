...
# Encountered twice
def collosion(lst, x, y):
    counted = []
    for cart in lst:
        if cart.get_pos() in counted and cart.get_pos() == (x,y):
            return True
        else:
...
if __name__ == "__main__":
    ...
    while len(carts) > 1:
        ind_to_remove = [] # carts to remove

        # Iterate carts in of L -> R, U -> D
        carts.sort(key=lambda c: c.y*maxx + c.x)
        for cart in carts:
            # Advance cart and check for collisions
            cart.advance(tracks[cart.get_pos()])

            # If a collosion occurs, remove carts at the collosion co-ord
            if collosion(carts, cart.x, cart.y):
                to_remove = (cart.x, cart.y) # pos to remove
                
                if DEBUG_GENERAL:
                    print("Removing carts at {0}".format(to_remove))

                # Search each cart and remove if necessary
                # Since carts are objects, they should be removed by memory
                for c_ind in range(len(carts)):
                    if carts[c_ind].get_pos() == to_remove:
                        ind_to_remove.append(c_ind)

                if DEBUG_GENERAL: # Print remaining positions
                    print([c.get_pos() for c in carts])
        # END for - note all ticks are completed

        # Remove carts which have collided (last first to prevent index issues)
        for ind in sorted(ind_to_remove, reverse=True):
            if DEBUG_GENERAL:
                print("Removing {0}".format(ind_to_remove))
            carts.pop(ind)
