...
if __name__ == "__main__":
    ...
    for row in range(len(board)):
        for col in range(len(board[row])):
            if board[row][col] == "E":
                elves.append(Combatant(col,row))
            elif board[row][col] == "G":
                goblins.append(Combatant(col,row))

    turn = 0 # Turn number. Incremented only when a turn is actually completed
    ...
    while len(elves)*len(goblins) > 0:
        ...
        for npc in full_list:
            ...
            if GAME_DEBUG:
                print("\nTurn of a ", ("goblin" if npc in goblins else "elf"))
            ...
            if len(target_list) > 0:
                ...
                if target != None: # If target found
                    ...
                    if target.hp <= npc.attack:
                        ...
                        full_list.remove(target)
                    else: # Else just take damage
                        target.take_damage(npc.attack)
        else: # Turn complete: increment turn counter
            turn += 1            

    print(turn, "rounds completed")

    # Print map
    for row in board:
        print("".join(row))
        
    print(sum([c.hp for c in elves + goblins])*turn)
